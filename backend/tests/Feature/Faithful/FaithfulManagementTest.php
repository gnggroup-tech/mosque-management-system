...
class FaithfulManagementTest extends TestCase
{
    ...
    {
        $superadmin = $this->user('superadmin'); $mosque = $this->mosque();
    ...
    {
        $superadmin = $this->user('superadmin'); $payload = $this->payload($this->mosque()); unset($payload['consent_at']);
    ...
    {
        $admin = $this->user('admin'); $other = $this->user('admin'); $own = $this->mosque($admin); $outside = $this->record($this->mosque($other));
    ...
    {
        $user = $this->user('user'); $own = $this->record($this->mosque(), $user); $this->record($this->mosque());
    ...
    {
        $superadmin = $this->user('superadmin'); $user = $this->user('user'); $existing = $this->record($this->mosque(), $user);
        $payload = $this->payload($this->mosque()); $payload['user_id'] = $user->id; $payload['registration_number'] = $existing->registration_number;
    ...
    {
        $superadmin = $this->user('superadmin'); $record = $this->record($this->mosque()); $record->update(['first_name' => 'Mamadou', 'status' => 'active']);
    ...
    {
        $admin = $this->user('admin'); $record = $this->record($this->mosque($admin));
        $this->actingAs($admin)->deleteJson(route('admin.faithful.destroy', $record))->assertNoContent();
        $this->assertSoftDeleted($record); $this->assertDatabaseHas('audit_logs', ['event' => 'faithful.deleted']);
    }

    public function test_guest_cannot_access_faithful_records(): void { $this->getJson(route('admin.faithful.index'))->assertUnauthorized(); }

    private function user(string $role): User { $user = User::factory()->create(); $user->assignRole($role); return $user; }
    private function mosque(?User $admin = null): Mosque { static $n = 0; $n++; return Mosque::query()->create(['code' => 'MOS-F'.str_pad((string) $n, 3, '0', STR_PAD_LEFT), 'name' => 'Mosquée '.$n, 'region' => 'Conakry', 'prefecture' => 'Conakry', 'commune' => 'Ratoma', 'status' => 'active', 'admin_id' => $admin?->id]); }
    private function payload(Mosque $mosque): array { return ['mosque_id' => $mosque->id, 'registration_number' => 'FID-0001', 'first_name' => 'Ibrahima', 'last_name' => 'Diallo', 'phone' => '620000000', 'joined_at' => '2026-08-01', 'consent_at' => '2026-08-01 10:00:00']; }
    private function record(Mosque $mosque, ?User $user = null): Faithful { static $n = 1; $n++; return Faithful::query()->create(['mosque_id' => $mosque->id, 'user_id' => $user?->id, 'registration_number' => 'FID-'.str_pad((string) $n, 4, '0', STR_PAD_LEFT), 'first_name' => 'Fidèle', 'last_name' => 'Test', 'joined_at' => '2026-08-01', 'consent_at' => now(), 'status' => 'active']); }
}
