...
class CouncilMemberManagementTest extends TestCase
{
    ...
    public function test_admin_can_manage_only_members_of_assigned_mosques(): void
    {
        ...
        $ownCouncil = $this->council($admin);
        $otherMember = $this->membership($this->council($other), $this->user('user'));
    ...
    {
        $superadmin = $this->user('superadmin');
        $council = $this->council(null, 'inactive');
        $this->actingAs($superadmin)->postJson(route('admin.council-members.store'), [
    ...
    {
        $user = $this->user('user');
        $membership = $this->membership($this->council(null), $user);
    ...
    {
        $admin = $this->user('admin');
        $membership = $this->membership($this->council($admin), $this->user('user'));
        ...
        $this->assertDatabaseHas('audit_logs', ['event' => 'council-member.deleted']);
    }

    public function test_guest_cannot_access_council_members(): void
    {
        $this->getJson(route('admin.council-members.index'))->assertUnauthorized();
    }
    ...
    private function membership(MosqueCouncil $council, User $user): CouncilMember
    {
        return CouncilMember::query()->create(['mosque_council_id' => $council->id, 'user_id' => $user->id, 'function' => 'imam', 'started_at' => '2026-01-01', 'status' => 'active']);
    }
}
