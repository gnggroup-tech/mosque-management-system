...
class CouncilMeetingManagementTest extends TestCase
{
    ...
    {
        [$admin] = $this->context(); [, $council, $members] = $this->context();
    ...
    {
        [$admin, $council, $members] = $this->context(); [, , $otherMembers] = $this->context();
    ...
    {
        [$admin, $council, $members] = $this->context(); $payload = $this->payload($council, $members); $payload['quorum_required'] = 3;
        $this->actingAs($admin)->postJson(route('admin.council-meetings.store'), $payload)->assertUnprocessable();
    }

    public function test_notice_can_be_sent_only_once(): void
    {
        [$admin, $council, $members] = $this->context(); $meeting = $this->meeting($admin, $council, $members);
    ...
    {
        [$admin, $council, $members] = $this->context(); $meeting = $this->meeting($admin, $council, $members);
        [, $otherCouncil, $otherMembers] = $this->context(); $this->meeting($otherCouncil->mosque->administrator, $otherCouncil, $otherMembers);
    ...
    private function payload(MosqueCouncil $council, array $members): array { return ['mosque_council_id' => $council->id, 'title' => 'Réunion ordinaire', 'agenda' => 'Finances et activités', 'scheduled_at' => now()->addDay()->toDateTimeString(), 'quorum_required' => 2, 'participant_ids' => array_map(fn ($m) => $m->id, $members)]; }
    private function meeting(User $admin, MosqueCouncil $council, array $members): CouncilMeeting { $response = $this->actingAs($admin)->postJson(route('admin.council-meetings.store'), $this->payload($council, $members))->assertCreated(); return CouncilMeeting::query()->findOrFail($response->json('id')); }
}
